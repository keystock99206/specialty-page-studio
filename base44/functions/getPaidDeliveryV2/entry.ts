...
export default async function(req:Request):Promise<Response>{
  try{
    ...
    if(!order.payment_entitlement_id){
      await base44.asServiceRole.entities.SecurityEvent.create({user_id:user.id,order_id:orderId,event_type:'payment_mismatch',severity:'high',details:{reason:'missing_entitlement',gate:'not_paid'},created_at:new Date().toISOString()});
    ...
    if(!entitlement||entitlement.order_id!==orderId){
      await base44.asServiceRole.entities.SecurityEvent.create({user_id:user.id,order_id:orderId,event_type:'payment_mismatch',severity:'high',details:{reason:'entitlement_missing_or_mismatched',gate:'not_paid'},created_at:new Date().toISOString()});
      return Response.json({error:'Payment has not been verified'},{status:403});
    }
    // Ownership gate: the entitlement must belong to the order's owner. A
    // mismatch is an access denial (not a payment failure) — distinct from
    // not-paid below. Both gates fail closed with 403 and no download_count
    // increment.
    if(entitlement.user_id!==order.created_by_id){
      await base44.asServiceRole.entities.SecurityEvent.create({user_id:user.id,order_id:orderId,event_type:'ownership_mismatch',severity:'high',details:{reason:'entitlement_not_owned',gate:'not_your_order',entitlement_user_id:entitlement.user_id,order_created_by_id:order.created_by_id},created_at:new Date().toISOString()});
      return Response.json({error:'Order access denied'},{status:403});
    }
    if(entitlement.payment_status!=='verified'||entitlement.production_authorized!==true||entitlement.proof_hash!==order.proof_hash){
      await base44.asServiceRole.entities.SecurityEvent.create({user_id:user.id,order_id:orderId,event_type:'payment_mismatch',severity:'high',details:{reason:'entitlement_invalid',gate:'not_paid',status:entitlement.payment_status,authorized:entitlement.production_authorized},created_at:new Date().toISOString()});
      return Response.json({error:'Payment has not been verified'},{status:403});
    }
  ...
  }catch(error:any){console.error('getPaidDeliveryV2 failed',error);return Response.json({error:error?.message||'Delivery failed'},{status:error?.status||500});}
}
