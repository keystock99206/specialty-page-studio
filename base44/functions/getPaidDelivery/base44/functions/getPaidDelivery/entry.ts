...
export default async function(req:Request):Promise<Response>{
  try{
    ...
    if(!entitlement||entitlement.order_id!==orderId||entitlement.user_id!==order.created_by_id||entitlement.payment_status!=='verified'||entitlement.production_authorized!==true||entitlement.proof_hash!==order.proof_hash){
      await base44.asServiceRole.entities.SecurityEvent.create({user_id:user.id,order_id:orderId,event_type:'payment_mismatch',severity:'high',details:{reason:'entitlement_invalid',status:entitlement?.payment_status||'missing',authorized:entitlement?.production_authorized},created_at:new Date().toISOString()});
      return Response.json({error:'Payment has not been verified'},{status:403});
    }
