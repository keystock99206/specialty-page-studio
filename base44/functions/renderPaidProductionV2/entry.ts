...
import { manifestsEqual } from '../../shared/pageComposition.ts';
import { acquireAuthoritativeThemeArt } from '../../shared/themeArtResolver.ts';
import { getOrderAuthority, applyOrderAuthorityEvent } from '../../shared/orderAuthority.ts';
import { compileAuthorityPackage } from './authorityPackageCompiler.ts';
...
export default async function(req:Request):Promise<Response>{
  try{
    ...
    // Product-master commercial authority is upstream of every production/release lane.
    ...
    // Certified package lane. A package order may release only from a registered,
    // immutable authority ZIP; otherwise the existing generic renderer remains separate.
    if(order.package_authority_id){
      const compiled=await compileAuthorityPackage(base44,{order,entitlement:ent,authorityId:order.package_authority_id,userId:orderOwnerId});
      return Response.json({
        status:'delivered',
        output_kind:'customer_package_zip',
        artifact_id:compiled.artifact.id,
        delivery_asset_id:compiled.delivery.id,
        page_count:compiled.artifact.page_count,
        file_sha256:compiled.artifact.file_sha256,
        package_authority_id:compiled.authority.id,
        package_key:compiled.authority.package_key,
        package_version:compiled.authority.package_version,
      });
    }
    ...
    if(priorOps.length&&order.production_artifact_id){
      ...
      return Response.json({status:'already_rendered',artifact_id:artifact?.id||order.production_artifact_id,page_count:artifact?.page_count||null,file_sha256:artifact?.file_sha256||null,render_ppi:artifact?.render_ppi||300});
    }
    ...
    const locks=mechanismIds.map((spId:string)=>{
      ...
      if(!['SP-013','SP-042','SP-084','SP-085','SP-117'].includes(spId)){
    ...
    // Production must consume the EXACT approved configuration. Recompute the
    // theme content hash and composition manifest from the current order +
    // theme + geometry and reject if either has drifted from the approved
    // full-product proof. This prevents reconstructing a different product
    // after payment.
    const approvedProof=await base44.asServiceRole.entities.ThemeProof.get(order.proof_id);
    if(!approvedProof)throw new Error('Approved full-product proof not found');
    const approvedSnap:any=(approvedProof as any).config_snapshot;
    if(!approvedSnap?.composition_manifest||!approvedSnap?.theme_content_hash)throw new Error('Approved proof is not a full-product proof');
    // Re-resolve the authoritative artwork (Drive connector / upload / AI) and
    // bind its sha256 into the theme-content hash. If the Drive asset changed
    // after approval, the hash drifts and production is rejected.
    const authoritativeArt = await acquireAuthoritativeThemeArt(base44, theme);
    const currentThemeHash=await shaJson({theme_name:theme.theme_name,theme_prompt:theme.theme_prompt,style:theme.style,preferred_colors:theme.preferred_colors,subject_focal_point:theme.subject_focal_point,text_content:theme.text_content,span_whole_bundle:theme.span_whole_bundle,cropping_permitted:theme.cropping_permitted,uploaded_art_sha256:theme.uploaded_art_sha256||null,drive_art_file_id:theme.drive_art_file_id||null,drive_art_sha256:authoritativeArt.sha256});
    if(currentThemeHash!==approvedSnap.theme_content_hash)return Response.json({error:'Approved theme configuration has changed since approval — a new proof and approval are required.'},{status:409});

    const rendered=await renderProductionPdf(base44,{mechanisms,theme,prefetchedArt:authoritativeArt});
    if(!rendered.geometryUnchanged||rendered.renderPpi!==300)throw new Error('Production render gate failed');
    if(!rendered.compositionManifest||!manifestsEqual(rendered.compositionManifest,approvedSnap.composition_manifest))return Response.json({error:'Production composition does not match the approved full-product proof — configuration drift detected.'},{status:409});
    ...
    const artifact=await base44.asServiceRole.entities.ProductionArtifact.create({order_id:orderId,order_number:order.order_number,user_id:orderOwnerId,is_test:isTest,entitlement_hash:ent.entitlement_hash,proof_hash:order.proof_hash,mechanism_ids:mechanismIds,engineering_hashes:locks.map((x:any)=>x.engineering_lock_sha256),geometry_bundle_hashes:locks.map((x:any)=>x.geometry_bundle_sha256),compositor_version:PRODUCTION_COMPOSITOR_VERSION,render_ppi:PRODUCTION_PPI,file_uri:uploaded.file_uri,file_sha256:rendered.productionSha256,page_count:rendered.pageCount,status:'verified',geometry_unchanged:true,qa_passed:true,created_from_entitlement:true});
    ...
    const release=await base44.asServiceRole.entities.ReleaseSnapshot.create({order_id:orderId,order_number:order.order_number,user_id:orderOwnerId,is_test:isTest,proof_hash:order.proof_hash,approval_hash:ent.approval_hash,entitlement_hash:ent.entitlement_hash,production_artifact_id:artifact.id,production_sha256:rendered.productionSha256,delivery_manifest_sha256:manifestHash,engineering_hashes:locks.map((x:any)=>x.engineering_lock_sha256),geometry_bundle_hashes:locks.map((x:any)=>x.geometry_bundle_sha256),released_at:new Date().toISOString(),status:'active'});
    const delivery=await base44.asServiceRole.entities.DeliveryAsset.create({order_id:orderId,order_number:order.order_number,user_id:orderOwnerId,is_test:isTest,production_artifact_id:artifact.id,file_uri:uploaded.file_uri,file_sha256:rendered.productionSha256,delivery_manifest_sha256:manifestHash,status:'ready',download_count:0});
    await base44.asServiceRole.entities.DriveArchiveOutbox.create({order_id:orderId,order_number:order.order_number,is_test:isTest,release_snapshot_id:release.id,production_artifact_id:artifact.id,source_file_uri:uploaded.file_uri,source_sha256:rendered.productionSha256,target_path:`Specialty Page Studio/Orders/${order.order_number}/specialty-pages-${order.order_number}-300ppi.pdf`,status:'blocked_connector',attempts:0,last_error:'Google Drive connector not authorized; customer delivery is unaffected.'});
    authority=await applyOrderAuthorityEvent(base44,{eventId:`release-bound:${orderId}:${artifact.id}`,orderId,eventType:'RELEASE_PACKAGE_BOUND',runtime:authority,payload:{production_artifact_id:artifact.id,production_sha256:rendered.productionSha256,release_snapshot_id:release.id}});
    authority=await applyOrderAuthorityEvent(base44,{eventId:`delivery-released:${orderId}:${delivery.id}`,orderId,eventType:'DELIVERY_RELEASED',runtime:authority,payload:{delivery_asset_id:delivery.id}});
  ...
  }catch(error:any){console.error('renderPaidProductionV2 failed',error);return Response.json({error:error?.message||'Entitled production failed'},{status:error?.status||500});}
}
