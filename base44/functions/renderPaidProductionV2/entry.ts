...
export default async function(req:Request):Promise<Response>{
  try{
    ...
    if(priorOps.length&&order.production_artifact_id){
      ...
      return Response.json({status:'already_rendered',artifact_id:artifact?.id||order.production_artifact_id,page_count:artifact?.page_count||null,file_sha256:artifact?.file_sha256||null,render_ppi:artifact?.render_ppi||null});
    }
    ...
    const currentThemeHash=await shaJson({theme_name:theme.theme_name,theme_prompt:theme.theme_prompt,style:theme.style,preferred_colors:theme.preferred_colors,subject_focal_point:theme.subject_focal_point,artwork_sha256:authoritativeArt.file_sha256});
    ...
    if(!rendered.compositionManifest||!manifestsEqual(rendered.compositionManifest,approvedSnap.composition_manifest))return Response.json({error:'Production composition does not match the approved proof — geometry or theme has drifted.'},{status:409});
